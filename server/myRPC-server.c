...
int is_user_allowed(const char *username) {
    ...
    if (!file) {
        perror("Не удалось открыть пользовательский файл");
        return 0;
    }
...
void execute_command(const char *command, char *result, int *code) {
    ...
    if (!stdout_fp || !stderr_fp) {
        perror("Не удалось открыть временные файлы");
        *code = 1;
        strcpy(result, "Не удалось открыть временные файлы");
        return;
    }
    int pid = fork();
    if (pid == 0) { 
        ...
        *code = 1;
        strcpy(result, "Ошибка создания потока");
        fclose(stdout_fp);
        fclose(stderr_fp);
        return;
    }
...
int main() {
    ...
    while (1) {
        ...
        if (is_user_allowed(username)) {
            ...
            code = 1;
            strcpy(result, "У пользователя нет доступа");
        }
